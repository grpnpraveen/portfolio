...
function StarCanvas({color_to}) {
  ...
  function movePointer(x, y) {
    if (typeof pointerX === "number" && typeof pointerY === "number") {
      ...
      velocity.tx = velocity.tx + (ox / 70) * scale * (touchInput ? 1 : -1);
      velocity.ty = velocity.ty + (oy / 70) * scale * (touchInput ? 1 : -1);
    }
  ...
  const render = (context) => {
    stars.forEach((star) => {
      ...
      context.lineWidth = STAR_SIZE * star.z * scale;
        ...
        context.strokeStyle =  color_to+ (0.5 + 0.5 * Math.random()) + ")";
